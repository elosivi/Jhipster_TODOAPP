...
interface StatusSearchRepositoryInternal {
    Stream<Status> search(String query);

    Stream<Status> search(Query query);
...
class StatusSearchRepositoryInternalImpl implements StatusSearchRepositoryInternal {
    ...
    @Override
    public Stream<Status> search(String query) {
        NativeQuery nativeQuery = new NativeQuery(QueryStringQuery.of(qs -> qs.query(query))._toQuery());
        return search(nativeQuery);
    }

    @Override
    public Stream<Status> search(Query query) {
        return elasticsearchTemplate.search(query, Status.class).map(SearchHit::getContent).stream();
    }
