...
export class SubTaskComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISubTask): number => this.subTaskService.getSubTaskIdentifier(item);

  search(query: string): void {
    if (query && SubTaskComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (SubTaskComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.subTasks = this.refineData(dataFromBody);
  }

  protected refineData(data: ISubTask[]): ISubTask[] {
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.subTaskService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
