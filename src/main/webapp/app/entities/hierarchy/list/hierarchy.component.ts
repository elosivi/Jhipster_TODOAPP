...
import { EntityArrayResponseType, HierarchyService } from '../service/hierarchy.service';
...
export class HierarchyComponent implements OnInit {
  ...
  trackId = (_index: number, item: IHierarchy): number => this.hierarchyService.getHierarchyIdentifier(item);

  search(query: string): void {
    if (query && HierarchyComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (HierarchyComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.hierarchies = this.refineData(dataFromBody);
  }

  protected refineData(data: IHierarchy[]): IHierarchy[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IHierarchy[] | null): IHierarchy[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.hierarchyService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.hierarchyService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
